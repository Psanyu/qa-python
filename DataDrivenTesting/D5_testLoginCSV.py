...
#Read csv file
csvfile = open("Plyrite/DataDrivenTesting/testdata/login.csv",newline='',encoding='utf-8')
reader= csv.DictReader(csvfile)

for row in reader:
    login_db.append((row["email"],row["password"],row["validity"]))

@pytest.mark.parametrize("email, password, validity",login_db)
